...
export default function StudentSelfDashboardShell() {
  ...
  return (
    <div className="space-y-4">
      ...
      <ForcePasswordChangeModal isOpen={forceOpen} onSkip={skip} onChanged={changed} />
      <Outlet />
    </div>
  );
}
...
function StudentSelfPrefetcher() {
  ...
  const studentId = auth?.user?.role === 'student' ? auth?.user?._id : null;
  ...
  useEffect(() => {
    ...
    queryClient.ensureQueryData({
      queryKey: ['studentSelfAttendance', studentId, from, to],
      queryFn: async () => {
    ...
    (async () => {
      try {
        ...
        if (academicYearId) {
          ...
          queryClient.ensureQueryData({
            queryKey: ['studentTranscriptByEnrollment', studentId, String(academicYearId), String(gradeSectionId)],
            queryFn: async () => {
        ...
        await queryClient.ensureQueryData({
          queryKey: ['timetableSlotsByGS', gradeSectionId],
          queryFn: async () => {
...
export function StudentSelfHomeCards({ studentIdOverride } = {}) {
  ...
  const studentId = studentIdOverride || (auth?.user?.role === 'student' ? auth?.user?._id : null);
  ...
  const timetableQuery = useQuery({
    queryKey: ['timetableSlotsByGS', activeGradeSectionId],
  ...
  const levelsQuery = useQuery({
    queryKey: ['studentLevelStats', studentId, enrollmentsKey],
    ...
    queryFn: async () => {
      ...
      for (const en of rows) {
        ...
        const { ok, data } = await getStudentTranscript(
          { academicYearId, gradeSectionId, studentId },
          { signal: sessionSignal }
        );
        if (!ok || !data) continue;
  ...
  return (
    <div className="space-y-4">
      {showWarmupLoading ? (
        <PageLoading title="Loading your dashboard…" subtitle="Preparing your attendance, timetable, and transcript" />
      ) : (
        <>
