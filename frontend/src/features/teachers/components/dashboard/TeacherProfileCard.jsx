...
function SmallStat({ label, value, tone = 'indigo' }) {
	...
	return (
		<div className={`rounded-lg border px-3 py-2 ${tones[tone] || tones.indigo}`}>
				...
				<div className="flex items-start gap-4">
					...
					<div className="min-w-0 flex-1">
						<div className="flex items-center gap-2">
							<div className="text-lg font-semibold text-gray-900 truncate">{fullName || 'Teacher'}</div>
						...
						<div className="mt-2 grid grid-cols-1 md:grid-cols-2 gap-2">
							...
							<div className="flex items-center gap-2 text-sm text-gray-700 min-w-0">
								...
								<span className="truncate">Teacher Ref: {teacherRef ? teacherRef : '—'}</span>
							</div>
							<div className="flex items-center gap-2 text-sm text-gray-700 min-w-0">
				...
				<div className="flex flex-col items-center text-center gap-4">
					...
					<div className="grid grid-cols-1 sm:grid-cols-3 gap-3 w-full max-w-3xl mt-3">
						<div className="rounded-lg p-4 bg-indigo-50 text-indigo-700 border border-indigo-100">
							...
							<div className="font-mono text-xl font-bold">{safeStr(user?.username)}</div>
						</div>
						<div className="rounded-lg p-4 bg-emerald-50 text-emerald-700 border border-emerald-100">
							...
							<div className="text-xl font-bold">{String(user?.role || 'teacher').toUpperCase()}</div>
						</div>
						<div className="rounded-lg p-4 bg-amber-50 text-amber-700 border border-amber-100">
							...
							<div className="font-mono text-xl font-bold">{user?.teacherRef ? String(user.teacherRef) : '-'}</div>
						</div>
					</div>
				</div>
			</div>
