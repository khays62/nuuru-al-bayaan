...
function CandidateTable({ rows = [], onTransfer, openingId, limit, total, onLimit }) {
  const STORAGE_KEY = 'transfers:candidates:columns:v1';
  const columns = [
    { key: 'studentId', label: 'Student ID' },
    { key: 'fullName', label: 'Full Name', tdClassName: 'px-6 py-4 text-sm font-medium text-gray-900 border-x border-gray-200' },
      ...
      renderCell={(st, col) => {
        ...
        switch (col.key) {
          ...
          case 'grade':
            return grade || '-';
          case 'section':
            return section != null ? `Sec ${section}` : '-';
          case 'shift':
            return shift || '-';
          case 'actions':
            return (
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        <TableState
          ...
          emptyDescription={candidatesCoreApplied
      ...
      <div className="bg-white rounded-lg shadow p-4">
        ...
        >
          <DataTable
            ...
            columns={[
              { key: 'date', label: 'Date' },
              { key: 'student', label: 'Student', tdClassName: 'px-6 py-4 text-sm font-medium text-gray-900 border-x border-gray-200' },
              { key: 'from', label: 'From' },
              { key: 'to', label: 'To' },
              { key: 'type', label: 'Type' },
            ...
            renderCell={(l, col) => {
              ...
              switch (col.key) {
                case 'date':
                  return dateStr;
                case 'student':
                  return (<>{l.student?.fullName || '-'} <span className="text-gray-500">({l.student?.studentId || ''})</span></>);
                case 'from':
                  return fromLbl || '-';
                case 'to':
                  return toLbl || '-';
