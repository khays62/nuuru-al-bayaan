...
import {
  ...
  Laptop, Globe, Clock, IdCard
} from "lucide-react";
import toast from "react-hot-toast";
import StatusBadge from "../components/common/badges/StatusBadge";
import { getUserById, getUserAuditLogs } from "../api";
...
export default function UserProfilePage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const userRes = await getUserById(userId);
      if (userRes.ok) setUser(userRes.data);  // ✅ this now points to the actual user object
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white rounded-xl shadow border">
        ...
        <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <Info label="Last Login" value={user.lastLogin ? format(user.lastLogin) : "Never"} icon={<Clock size={18} />} />
