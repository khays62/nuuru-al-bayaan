...
export default function AttendanceReportsPage() {
  ...
  const canViewReports =
    hasPermission('attendanceReports', 'view')
    || hasPermission('attendanceReports', 'print')
    || hasPermission('attendanceReports', 'download')
  ...
  const canDownloadReports =
    hasPermission('attendanceReports', 'download')
    // Backward compatibility
    || hasPermission('attendance', 'download');

  const canPrintReports =
    hasPermission('attendanceReports', 'print')
    // Backward compatibility
    || hasPermission('attendance', 'print');
  ...
  async function runSummary() {
    if (!canViewReports) {
      toast.error('You don’t have permission to view attendance reports');
      return;
    }
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      {!canViewReports && (
        <div className="text-sm text-gray-600 no-print">You don’t have permission to view attendance reports.</div>
      )}
