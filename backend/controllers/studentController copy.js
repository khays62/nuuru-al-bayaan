...
// //     if (!studentId) {
// //       return res.status(400).json({ message: "Student not found in session" });
// //     }

// //     // Find all results for this student
// //     const results = await ExamScore.find({ student: studentId })
...
// //       const results = await ExamScore.find({ student: studentId })
// //         .populate("subject", "name code")
// //         .populate("exam", "name term academicYear") // 👈 add exam details
// //         .sort({ createdAt: -1 });
...
// //         groupedResults[subjectName].push({
// //           examName:
// //             `${result.exam?.name || "Unknown Exam"} (${result.exam?.term || ""} ${result.exam?.academicYear || ""})`,
// //           score: result.score,
// //           grade: result.grade,
// //         });
// //       }
  
// //       return res.json({ success: true, results: groupedResults });
// //     } catch (err) {
// //       console.error("❌ Error fetching student results:", err);
// //       return res.status(500).json({ message: "Server error" });
// //     }
...
// //       // ✅ Make sure to populate both subject and exam fields properly
...
// //         if (!groupedResults[subjectName]) groupedResults[subjectName] = [];
  
// //         groupedResults[subjectName].push({
// //           examName: score.exam?.name || "Unknown Exam",
// //           score: score.scoreObtained,
// //           grade: getGrade(score.scoreObtained), // helper below
// //         });
// //       }
...
// //         const subjectName = score.subject?.name || "Unknown Subject";
// //         const examName = score.exam?.name?.toLowerCase() || "unknown";
  
// //         if (!results[subjectName]) results[subjectName] = { midterm: 0, final: 0, total: 0 };
  
// //         if (examName.includes("mid")) results[subjectName].midterm = score.scoreObtained;
// //         else if (examName.includes("final")) results[subjectName].final = score.scoreObtained;
  
// //         results[subjectName].total =
// //           results[subjectName].midterm + results[subjectName].final;
// //       });
...
// //       const scores = await ExamScore.find({ student: studentId })
// //         .populate("subject", "name")
// //         .populate("exam", "name scoreType"); // make sure your Exam model has 'name'
...
// //       // Populate subjectName and exam.examType.name
// //     //   const scores = await ExamScore.find({ student: studentId })
// //     //     .populate({
// //     //       path: "exam",
// //     //       populate: { path: "examType", select: "name" }, // nested populate
// //     //     })
...
// //         const examTypeName = score.exam?.examType?.name?.toLowerCase() || "";
  
// //         if (!results[subjectName]) {
// //           results[subjectName] = { midterm: 0, final: 0, total: 0 };
// //         }
  
// //         if (examTypeName.includes("mid")) {
// //           results[subjectName].midterm = score.scoreObtained;
// //         } else if (examTypeName.includes("final")) {
// //           results[subjectName].final = score.scoreObtained;
// //         }
...
// // export const getMyResults = async (req, res) => {
// //   try {
// //     const studentId = req.user._id; // or req.user.id depending on your auth middleware
...
// //     for (const score of scores) {
// //       const subjectName = score.subject?.subjectName || "Unknown Subject";
// //       const examTypeName = score.exam?.examType?.name?.toLowerCase() || "unknown";

// //       if (!results[subjectName]) {
// //         results[subjectName] = { midterm: 0, final: 0, total: 0 };
// //       }

// //       // Detect exam type and assign score
// //       if (examTypeName.includes("mid")) {
// //         results[subjectName].midterm = score.scoreObtained;
// //       } else if (examTypeName.includes("final")) {
// //         results[subjectName].final = score.scoreObtained;
...
// //       // Update total
// //       results[subjectName].total =
// //         (results[subjectName].midterm || 0) +
// //         (results[subjectName].final || 0);
// //     }
