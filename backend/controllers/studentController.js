...
export const getMyResults = async (req, res) => {
  try {
    ...
    const studentInfo = {
      ...
      id: student._id,
      grade: enrollment?.gradeSection?.grade?.gradeName || "—",
    ...
    for (const score of scores) {
      ...
      if (examType.includes("mid")) results[subjectName].midterm = score.scoreObtained;
      else if (examType.includes("final")) results[subjectName].final = score.scoreObtained;

      results[subjectName].total = (results[subjectName].midterm || 0) + (results[subjectName].final || 0);
    }
    ...
    res.json({
      ...
      results,
      average,
      rank: null, // rank logic can be added later
    });
  } catch (error) {
    console.error("❌ Error fetching results:", error);
    res.status(500).json({ message: "Failed to load results" });
